...
export function StockMovementClient() {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => setUnitCostStr(c > 0 ? String(c) : ""));
  }, [selectedItem, mode]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let dateBs: string;
    try {
      ({ date, dateBs } = datePairFromAdYmd(form.date));
    } catch {
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <div className="mt-1 text-sm text-slate-500">
                        <DualDateDisplay iso={dateIso || undefined} dateBs={m.dateBs} layout="inline" />
                        {dateIso.includes("T") && dateIso.length > 10 ? ` · ${time}` : null}
                      </div>
                    </div>
                    <div
